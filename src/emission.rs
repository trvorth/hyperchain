...
impl Emission {
    ...
    pub fn update_supply(&mut self, reward: u64) -> Result<(), String> {
        ...
        debug!(
            ...
            self.current_supply, actual_reward_added_to_supply
        );
        Ok(())
    }
...
mod tests {
    ...
    fn test_calculate_reward() {
        ...
        let mut emission_single = Emission::new(
            INITIAL_REWARD,
            TOTAL_SUPPLY,
            HALVING_PERIOD,
            HALVING_FACTOR,
        ...
        let reward_after_one_halving = emission_single
            .calculate_reward(timestamp_after_one_halving)
            .unwrap();
        // Corrected: Explicitly cast to f64 for round
        let expected_after_one_halving =
            ((INITIAL_REWARD as f64 * HALVING_FACTOR * SCALE as f64).round() / SCALE as f64) as u64
                / num_chains_single as u64;
        ...
        let mut emission_multi = Emission::new(
            INITIAL_REWARD,
            TOTAL_SUPPLY,
            HALVING_PERIOD,
            HALVING_FACTOR,
        ...
        assert_eq!(
            reward_multi_genesis,
            (INITIAL_REWARD / num_chains_multi as u64).max(1)
        );
        emission_multi.update_last_halving_period(genesis_timestamp);

        let reward_multi_one_halving = emission_multi
            .calculate_reward(timestamp_after_one_halving)
            .unwrap();
        let expected_multi_one_halving =
            (((INITIAL_REWARD as f64 * HALVING_FACTOR).round() as u64) / num_chains_multi as u64)
                .max(1);
    ...
    fn test_update_supply() {
        ...
        assert!(
            result.is_err(),
            "Should return an error if total supply cap is hit/exceeded during update"
        );
        assert_eq!(
            emission.current_supply(),
            1000,
            "Supply should be capped at total_supply"
        );
    }
    ...
    fn test_quantum_resistant_adjustment() {
        ...
        let now_for_test = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_secs();
        ...
        assert!(
            ...
            "Adjusted reward {} should be >= base reward {}",
            adjusted,
            current_reward
        );
        // The upper bound is more complex due to the modulo SCALE, but it should be related to base_reward + SCALE
        assert!(
            adjusted <= current_reward + SCALE,
            "Adjusted reward {} is unexpectedly high relative to base {} and SCALE {}",
            adjusted,
            current_reward,
            SCALE
        );
