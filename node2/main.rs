...
#[derive(Parser, Debug)]
#[clap(author, version, about = "HyperDAG Test Node 2")]
struct Args {
    #[clap(long, default_value = "./node2/node2_config.toml")]
    config_path: PathBuf,
}
...
async fn main() -> Result<()> {
    let args = Args::parse();
    let log_prefix = "[Node 2]";

    // --- Path Definitions ---
    let config_path = &args.config_path;
    let identity_key_path = "./node2/p2p_identity.key";
    let wallet_path = "./node2/node2_wallet.key";
    let peer_cache_path = "./node2/peer_cache.json";
    ...
    let node_handle = tokio::spawn(async move {
        if let Err(e) = node_instance.start().await {
            error!("[Node 2] Node task failed: {e}");
        }
    });
